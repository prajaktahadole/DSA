...
var min=-1;
var max =-1;
function MasaiPackersMoversII(k,str,index=0){
    ...
    else if(k===0 && index>max){
        max = index;
    }
    else if(k==0 && index<min){
        min = index;
    }
    if(k<0){
        return;
    ...
    for(let i = 0;i<str.length;i++){
        MasaiPackersMoversII(k-str[i],str,index+1)
    }
...
function runProgram(input){
   ...
   var ans = MasaiPackersMoversII(k,str);
   if(max == -1)
   {
       console.log(max);
   }
